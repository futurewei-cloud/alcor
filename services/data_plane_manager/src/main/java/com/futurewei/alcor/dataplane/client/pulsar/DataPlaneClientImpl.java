...
public class DataPlaneClientImpl implements DataPlaneClient {
    ...
    public void createGoalState(List<UnicastGoalState> unicastGoalStates) throws Exception {
        for (UnicastGoalState unicastGoalState: unicastGoalStates) {
            String topic = topicManager.getTopicByHostIp(unicastGoalState.getHostIp());

            Producer<GoalState> producer = pulsarClient.newProducer(Schema.PROTOBUF(GoalState.class))
                    .topic(topic)
                    .enableBatching(false)
                    .create();

            producer.send(unicastGoalState.getGoalState());
        }
    }
    ...
    public void createGoalState(MulticastGoalState multicastGoalState) throws Exception {
        String topic = topicManager.getMulticastTopic();
        Producer<MulticastGoalState> producer = pulsarClient.newProducer(JSONSchema.of(MulticastGoalState.class))
                .topic(topic)
                .enableBatching(false)
                .create();
