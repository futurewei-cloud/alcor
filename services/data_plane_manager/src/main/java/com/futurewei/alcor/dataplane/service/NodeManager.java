...
public class NodeManager {
    ...
    public NodeManager(List<HostInfo> hosts) {
        ...
        logger.entering(this.getClass().getName(), "NodeManager" +
                "(List<HostInfo> hosts)");

        this.nodes = LoadNodes(hosts);
        for (HostInfo host : hosts) {
            System.out.println(host);
            logger.log(Level.INFO, "Log:" + host);
        }
        this.BuildMapFromNodeIdToInfo(this.nodes);
        logger.exiting(this.getClass().getName(), "NodeManager(List<HostInfo>" +
                " hosts)");
...
//            host.setGRPCServerPort(NodeManager.GRPC_SERVER_PORT); // not sure
        }
    ...
    public HostInfo getHostInfoById(String hostId) {
        ...
        if (this.nodeMap != null) { //&& !Strings.isNullOrEmpty(hostId)) {
            logger.log(Level.INFO,
                    "Log:" + "[NodeManager] Host id: " + hostId + " info:" + this.nodeMap.get(hostId));
    ...
    public HostInfo[] getRandomHosts(int count) {
        ...
        for (int i = 0; i < count; i++) {
            int index = Common.getRandomNumberInRange(0,
                    this.getNodes().size() - 1);
    ...
    private void BuildMapFromNodeIdToInfo(List<HostInfo> hosts) {
        ...
        if (hosts != null) {
            ...
            for (HostInfo host : hosts) {
                this.nodeMap.put(host.getIpAddress(), host);
            }
        }
    }
}
