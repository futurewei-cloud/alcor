...
|Verify IP State
|GET
|/ips/{ip_version}/{subnet_id}/{ip}
|ip state
<<IP_Get,[sample]>>

|List all allocated IP State
|GET
|/ips/{ip_version}/{subnet_id}
|ip state list
<<IP_List,[sample]>>
...
* Request: `/ips/{ip_version}/{subnet_id}/{ip}`

* Request Parameter: `@PathVariable int ipVersion, @PathVariable String subnetId, @PathVariable String ip`
...
* Request: `/ips/{ip_version}/{subnet_id}`

* Request Parameter: `@PathVariable int ipVersion, @PathVariable String subnetId`
...
....
Request:
http://127.0.0.1:8080/ips/174ac5e4-7fb5-11ea-8cc4-000c29f4bc8b
