...
* Request: `/project/{projectid}/routers/{routerid}`
* Request Parameter: `@PathVariable String projectid, @PathVariable String routertid`
* Action: Deletes a logical router and, if present, its external gateway interface. (#need to talk PM about the port,
router only has gateway ip and subnet_id#)
...
** Subnet ID. The gateway IP address for the subnet is used as an IP address of the created router interface. (#need to get gw port id#)
** Port ID. The IP address associated with the port is used as an IP address of the created router interface. (#PM need to know is the port has been used#)
...
If the port is already in use, this operation returns the Conflict (409) response code. (#PM need to provide this information#)
...
* Others: After you run this operation, the operation sets: (#notify PM to do that#)
...
* Action: Delete an internal router interface, which detach a subnet from the router.
* Response: Router's state
* Normal response codes: 200
* Error response codes: 400, 401, 404, 409
...
* Response: If you specify both subnet ID and port ID, the subnet ID must correspond to the subnet ID of the first IP address on the port.
