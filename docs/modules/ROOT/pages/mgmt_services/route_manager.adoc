...
anchor:subnet_get_routetable[]
**(16) Show Subnet route table**
...
** If the routing table doesn't exist, this operation will create a routing table (call API for creating subnet route table).
* Response: Routing table state
* Normal response codes: 200
* Error response codes: 400, 401, 404, 500
* Example
...
**(17) Create Subnet route table**

* Method: `POST`
* Request: `/project/{projectid}/subnets/{subnetid}/routetable`
* Request Parameter: `@PathVariable String projectid, @PathVariable String subnetid`
* Operation: Create subnet's routing table.
** If the subnet type is *Neutron*, push default gateway routing rule into route table,
e.g.: { "destination": "0.0.0.0/0", "nexthop": "192.168.0.1", "priority": -1 }. (#may not needed#)
** If the subnet type is *public-subnet* or *private-subnet*, push-in VPC's default routing rules (#may not needed#).
** #The above operations can be deferred to when Subnet Manager pushing subnet's routing rules to DPM/ACA#
* Response: Routing table state
* Normal response codes: 200
* Error response codes: 400, 401, 404, 500
* Example
