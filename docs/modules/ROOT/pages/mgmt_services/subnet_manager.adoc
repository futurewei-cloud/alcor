...
2. Handle Subnet level routing rules updating
...
We need a cache to maintain subnet-port-host mapping and node-nodeInfo mapping HashMap<subnet_id, HashMap<port_id, host_id>>HashMap<node_id, NODEINFO>
Cache can be an ignite cache for PM, PM has subnet-port-host persistent info, it’s easy for PM to provide a fast-path API for the mappings.
Cache can be an ignite cache for DPM, but DPM needs to ask PM when there is cache-miss, the cache synchronization is another issue.
Local cache or external cache? Pros and cons?


50 ports/subnet, 50x2M subnets = 100M x 20Byte  = 2GB
200B x 1M nodes = 200MB
