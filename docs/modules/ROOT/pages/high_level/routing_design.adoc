...
Note, Aliyun mostly use highlevel programming languages: Java/Python/Go/PHP: https://github.com/aliyun, similar thing for tencent cloud
...
We will move away from neutron's which requires resources for the namespace, latency of using linux devices and TCP stack, overhead of using iptables. We will use the more modern openflow rules + openflow controller application to implement our virtual router, which consumes much less resource and provides high routing performance because it doesn't need to go through the full kernel TCP stack.
...
A virtual router will be implemented as part of ACA. When Alcor Controller sends down the goal state messages to compute hosts, ACA will create the in memory virtual router object and also the (virtual) gateway ports connected to it. ACA will then configure routing rules in the virtual router object. The last step ACA will do is to program openflow rules so that packets destinated to virtual router's gateway interface will be send to openflow controller that's ACA in our case.
